<template>
    <v-card >
        <v-app-bar flat >
            <v-card-title class="ma-0 pa-0">{{type == 'survey' ? '설문내역' : `주문내역 (${items?.length ?? 0})`}}</v-card-title>
        </v-app-bar>
        <v-container class="user_history_main_container ma-0 pa-5" v-if="type == 'survey'">
            <v-data-table 
                :items="items"
                :header="surveyHeaders"
                :items-per-page="10"
                disable-sort
                ...
                <v-stepper-items class="ma-0 pa-0 mt-1">
                    ...
                    >
                        <v-card
                            ...
                            v-if="i < 5"
                        >
                            <v-row class="ma-0 pa-0 my-2 align-center" v-for="(listItem, index) in obj.stepList" :key="index">
                                {{listItem.text}}
                                <v-spacer />
                        ...
                        <v-card v-else class="ma-0 pa-4 overflow-auto" >
                            ...
                            <v-divider class="ma-0 pa-0 mt-4" style="border-color:black; border-width: 2px 0 0 0;"/>
                            <v-row class="ma-0 pa-0 align-center" style="height:56px; background-color:#fbfbfd;">
        ...
        <v-container class="ma-0 pa-0 user_history_main_container" v-else >
            <v-data-table 
                :items="items"
            ...
            >
                <template v-slot:[`item.b_name`]="{item}">
                    <div v-if="item?.order_list.length > 0">
                        {{JSON.parse(item?.order_list[0]).product?.name ?? ""}} 외 {{item?.order_list.length}}건
                    </div>
                </template>
        ...
        <v-footer class="">
            <v-btn text @click="clickPrev" v-if="type=='survey'" :disable="step == 0">
                Prev
            </v-btn>
            <v-btn
                class="ml-2"
...
export default {
    ...
    data() {
        return {
            ...
            orderHeaders: [
                {text:"제품명", value:"b_name", align:"center"},
                {text:"금액", value:"total_price", align:"center"},
                {text:"구매일자", value:"cd", align:"center"},
            ],
            stepList: [
                {
                    stepList: [
    ...
    methods: {
            ...
            if (this.obj != null) {
                ...
                await this.$axios.post('/admin/select', param).then(async res => {
                    if (res.data.length > 0) {
                        ...
                        if (this.type == 'order') this.selectOrderList()
                        else if (this.type == 'survey') this.selectSurveyList()
                    }
                }).catch(err => {
                    console.log("err : ", err)
                })
        ...
        async selectOrderList() {
            let conditions = [{ q: '=', f: 'b_no', v: this.userObj?.no },{ "op":"AND",q: '=', f: 'status', v: 7 },{ q: 'order', f: 'cd', o: 'desc' }]
            let orderFormBody = {
                table: 'orders',
                conditions: conditions,
            }
            this.$axios.post('/admin/select', orderFormBody).then(res => {
                if (res.data.length > 0) {
                    this.items = res.data
                    for(let one of this.items){
                        one.cd = Moment(one.cd).format('YYYY-MM-DD HH:mm')
                        if (one.order_list != null && one.order_list != undefined) one.order_list = JSON.parse(one.order_list)
                    }
                }
                this.$forceUpdate()
            }).catch(err => {
